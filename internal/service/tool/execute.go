...
func (s *Service) Execute(ctx context.Context, td *models.ToolDefinition, in map[string]any) (int, []byte, error) {
	...
	q := u.Query()
	effectiveInput := withV1QueryAliases(in)
	...
	for _, p := range td.Params {
		...
		if v, ok := effectiveInput[p.Name]; ok && v != nil {
...
func withV1QueryAliases(in map[string]any) map[string]any {
	...
	if _, ok := out["limit"]; !ok {
		if v, ok := out["recPerPage"]; ok {
			out["limit"] = v
		}
	}

	if _, ok := out["page"]; !ok {
		if v, ok := out["pageID"]; ok {
			out["page"] = v
		}
	}
