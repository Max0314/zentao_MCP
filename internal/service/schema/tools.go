...
func (s *Service) buildTool(ctx context.Context, doc *openapi3.T, p, method string, op *openapi3.Operation, pi *openapi3.PathItem) models.ToolDefinition {
	...
	desc := strings.TrimSpace(firstNonEmpty(
		...
		input := s.buildToolInputSchema(ctx, doc, p, op, pi)
...
func collectParams(op *openapi3.Operation, pi *openapi3.PathItem, path string) []models.ToolParam {
	...
	for _, name := range pathParamNames(path) {
		...
		seen[key] = true
		params = append(params, models.ToolParam{Name: name, In: "path"})
	}
